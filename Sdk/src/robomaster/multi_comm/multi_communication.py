...
class MultiCommunication(object):
    ...
    def __recv_callback_process(self):
        while not self.__recv_callback_process_finish and not self.event_client.script_state.check_stop() or  self.event_client.script_state.check_script_has_stopped():
            try:
                msg = self.recv_msg_queue.get()
                # if self.__recv_callback_process_finish or self.event_client.script_state.check_stop():
                if self.__recv_callback_process_finish or self.event_client.script_state.check_stop() or  self.event_client.script_state.check_script_has_stopped():
                    break

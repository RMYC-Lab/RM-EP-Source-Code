...
def stop_script_thread(ident):
    print('thread.ident:',ident," status:",SystemExit)
...
def m_sum(m1, m2):
    row = len(m1)
    column = len(m1[0])
    m = [[m1[i][j] + m2[i][j] for j in range(column)] for i in range(row)]
    return m
...
def submatrix_44(it, jt, m):
    tmpm = [[m[i][j] if i != it and j != jt else 'null' for j in range(4)] \
            for i in range(4)]
    tmpm.remove(['null'] * 4)
    for i in range(4):
        try:
...
# 4_4 matrix determinant
def det_44(m):
    m0 = submatrix_44(0, 0, m)
    m1 = submatrix_44(1, 0, m)
    m2 = submatrix_44(2, 0, m)
    m3 = submatrix_44(3, 0, m)
    det = m[0][0] * det_33(m0) - m[1][0] * det_33(m1) + \
          m[2][0] * det_33(m2) - m[3][0] * det_33(m3)
    return det


# inverse matrix 4_4
def inv_44(m):
    adjm = [[det_33(submatrix_44(i, j, m)) * (-1) ** (i + j) for i in range(4)] for j in range(4)]
    det = det_44(m)
    M = [[1 / det * adjm[j][i] for i in range(4)] for j in range(4)]
    return M
...
def dot(m1, m2):
    row = len(m1)
    column = len(m2[0])
    M = [[[m1[i][k] * m2[k][j] for k in range(len(m2))] for j in range(column)] for i in range(row)]
    M = [[functools.reduce(lambda x, y:x+y, M[i][j]) for j in range(column)] for i in range(row)]
    return M
...
def transpose(m):
    row = len(m[0])
    column = len(m)
    M = [[m[j][i] for j in range(column)] for i in range(row)]
    return M
...
class Chassis(object):
    ...
    def set_position_cmd(self, ctrl_mode, axis_mode, pos_x, pos_y, angle_yaw, vel_xy, alg_omg, cmd_type=rm_define.NO_TASK):
        ...
        if cmd_type == rm_define.TASK:
            ...
            duss_result, identify = self.event_client.send_task_async(self.msg_buff, event_task, time_out=2)
...
class Gimbal(object):
    ...
    def set_degree_ctrl(self, pitch_degree, yaw_degree, pitch_accel, yaw_accel, axis_maskbit, coodrdinate, deviation = rm_define.gimbal_deviation, cmd_type=rm_define.NO_TASK):
...
class Aimodule(object):
    def __init__(self, event_client):
        self.event_client = event_client
        self.msg_buff = duss_event_msg.EventMsg(tools.hostid2senderid(event_client.my_host_id))
        self.msg_buff.set_default_receiver(rm_define.ai_id)
        self.msg_buff.set_default_moduleid(rm_define.system_id)
        self.msg_buff.set_default_cmdset(duml_cmdset.DUSS_MB_CMDSET_RM)

    def recognition_event_register(self, callback):
        cmd_set_id = duml_cmdset.DUSS_MB_CMDSET_RM << 8 | duml_cmdset.DUSS_MB_CMD_RM_AI_MODULE_INFO_PUSH
        self.event_client.async_req_register(cmd_set_id, callback)
    ...
    def open_ai_set_addr(self):
        ...
        receiver = 0x0301
        addr = 0x0103
        cmd_type = 0 #no need ack
        cmd_id = 0x20D #20D
        crc_h_data = [0xAA] + [pack_size & 0xff] + [pack_size >> 8]
        seq_num = random.randint(0, 1000)
        crc_h_t = duml_crc.duss_util_crc8_append(crc_h_data, rm_define.DUSS_MB_PACKAGE_OPEN_CRCH_INIT)
        crc_data = [0xAA] + [verlen&0xff] + [verlen>>8] + crc_h_t + [sender&0xff] + [sender>>8] + [receiver&0xff] +[receiver>>8] + [cmd_type] +\
                   [seq_num&0xff] + [seq_num >> 8] + [cmd_id&0xff] + [cmd_id >> 8] +  [sender&0xff]+ [sender>>8]
        crc_t = duml_crc.duss_util_crc16_append(crc_data, rm_define.DUSS_MB_PACKAGE_OPEN_CRC_INIT)

        self.msg_buff.append('attr', 'uint8', 0xAA) # sof
        self.msg_buff.append('value', 'uint16', pack_size)
        self.msg_buff.append('crc', 'uint8', crc_h_t[0])
        self.msg_buff.append('sender', 'uint16', sender)
        self.msg_buff.append('receiver', 'uint16', receiver)
        self.msg_buff.append('type', 'uint8', cmd_type)
        self.msg_buff.append('seq_num', 'uint16', seq_num)
        self.msg_buff.append('cmd_id', 'uint16', cmd_id)
        self.msg_buff.append('addr', 'uint16', addr)
        self.msg_buff.append('crc16_l', 'uint8', crc_t[0])
        self.msg_buff.append('crc16_h', 'uint8', crc_t[1])

        self.msg_buff.receiver = rm_define.can_reciver
        self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_RM_AI_MODULE_SET
        #self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_RM_AI_MODULE_INFO_PUSH
        duss_result, resp = self.event_client.send_sync(self.msg_buff)
        return duss_result

    def open_ai_set_mode(self, mode_type=rm_define.AI_USER_MOD):
        self.msg_buff.init()
        pack_size = 1
        verlen = 1
        sender = 0x0103
        ...
        crc_h_t = duml_crc.duss_util_crc8_append(crc_h_data, rm_define.DUSS_MB_PACKAGE_OPEN_CRCH_INIT)
        crc_data = [0xAA] + [verlen&0xff] + [verlen>>8] + crc_h_t + [sender&0xff] + [sender>>8] + [receiver&0xff] +[receiver>>8] + [cmd_type] +\
                   [seq_num&0xff] + [seq_num >> 8] + [cmd_id&0xff] + [cmd_id >> 8] + [idx&0xff]
        crc_t = duml_crc.duss_util_crc16_append(crc_data, rm_define.DUSS_MB_PACKAGE_OPEN_CRC_INIT)
        ...
        self.msg_buff.append('seq_num', 'uint16', seq_num)
        self.msg_buff.append('cmd_id', 'uint16', cmd_id)
        self.msg_buff.append('addr', 'uint8', idx)
        self.msg_buff.append('crc16_l', 'uint8', crc_t[0])
        self.msg_buff.append('crc16_h', 'uint8', crc_t[1])
    ...
    def play_sound(self, id):
        ...
        if duss_result != rm_define.DUSS_SUCCESS:
            if id >= rm_define.media_custom_audio_0 and id <= rm_define.media_custom_audio_9:
                raise Exception('SCRIPT_CTRL: play rm_define.media_custom_audio_%d failed, raise exception'%(id-rm_define.media_custom_audio_0))
...
class SDKModule(object):
    ...
    def game_push_on(self, callback):
        cmd_set_id = duml_cmdset.DUSS_MB_CMDSET_RM << 8 | duml_cmdset.DUSS_MB_CMD_RM_YOUTH_COMPETITION_DATA_PUSH
        self.event_client.async_req_register(cmd_set_id, callback)
    ...
    def sdk_function_ctrl(self, callback):
        cmd_set_id = duml_cmdset.DUSS_MB_CMDSET_RM << 8 | duml_cmdset.DUSS_MB_CMD_RM_YOUTH_COMPETITION_SDK_CTRL
        self.event_client.async_req_register(cmd_set_id, callback)

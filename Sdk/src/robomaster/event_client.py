...
class EventClient(object):
            ...
            if msg['ack'] == True:
                ...
                try:
                    ...
                    if cmd_set_id in self.async_req_cb_list.keys():
                        ...
                        logger.warn('UNSUPPORT MSG CMD SET = ' + str(hex(msg['cmd_set'])) + ', CMD ID = ' + str(hex(msg['cmd_id'])))
                except Exception as e:
                    logger.error('cb running traceback error  message')
                    # logger.error(e)
    ...
    def foreach_item(self):
        for i , (key , cb)  in enumerate( self.async_req_cb_list.items()):
            logger.error(' foreach_item idex:'+ str(i) + " , " + str(hex(key)) + " , "+ str(cb) )
    ...
    def send_sync(self, event_msg, time_out = duml_cmdset.MSG_DEFAULT_TIMEOUT):
        ...
        if event_msg.cmd_type & duml_cmdset.NEED_ACK_TYPE:
            ...
            if not self.wait_ack_event_list[j].wait_ack_event.isSet():
                ...
                logger.warn('CMDSET = ' + str(hex(event_msg.cmd_set)) + ', CMDID = ' + str(hex(event_msg.cmd_id)) + ' TIMEOUT')

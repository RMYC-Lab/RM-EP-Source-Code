...
class ProtocalParser(object):
    ...
    def init(self, config={}):
        ...
        self.sdk_ctrl.sdk_function_ctrl()
    ...
    def __command_recv_callback(self, fd, data):
        if self.connection_obj == self.uart_obj:
            ...
            if self.sdk_ctrl.get_func_flag() == False:
                logger.info('Can not use sdk, the function is disable')
                return
    ...
    def sdk_robot_ctrl(self, ctrl):
        def init():
            ...
            self.ctrl_obj['AI_ctrl'] = rm_ctrl.VisionCtrl(self.ctrl_obj['event'], self.ctrl_obj['gimbal_ctrl'])
    ...
    def __data_process(self):
        last_sdk_func_flag = True
        self.sdk_robot_ctrl('init')
        self.sdk_robot_ctrl('ready')

        while self.sdk_mode:
            if self.sdk_ctrl.get_func_flag() == False:
                if last_sdk_func_flag != self.sdk_ctrl.get_func_flag():
                    logger.info('sdk robot control stop')
                    self.sdk_robot_ctrl('stop')
                else:
                    logger.info('sdk robot control already stop')
                last_sdk_func_flag = False
            else:
                last_sdk_func_flag = True
    ...
    def io_level_event_push_callback(self, event):
        ...
        if 'io_level' in event.keys():
            msg += ' io_level %d %d %d;'%(event['io_level'])
        self.send('event', msg)
